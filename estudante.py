...
class CtrlEstudante():       
    ...
    def consultaEstudantes(self):
        self.limiteIns = LimiteConsultaEstudante(self)
    ...
    def consultaHandler(self, event):
        ...
        for estudante in self.listaEstudantes:
            if nroMatric == estudante.getNroMatric():
                str = 'Nro Matric. -- Nome\n'
                str += estudante.getNroMatric() + ' -- ' + estudante.getNome() + '\n' 
                self.limiteLista = LimiteMostraEstudantes(str)
                confirma = 1
            
        if confirma == 0:
            self.limiteIns.mostraJanela('Erro', 'Estudante não cadastrado no sistema')
